            ...
            return (
              <BlurView key={net.id} intensity={30} tint="dark" style={styles.networkCard}>
                ...
                <View style={styles.statsRow}>
                  <View style={styles.statItem}><Feather name="layers" size={14} color="#BF5AF2" /><Text style={styles.statText}>{net.subnets.length} Subredes</Text></View>
                  <View style={styles.statItem}><Feather name="cpu" size={14} color="#30D158" /><Text style={styles.statText}>{net.device_count || 0} Equipos</Text></View>
                  <View style={styles.statItem}><Feather name="shield" size={14} color="#0A84FF" /><Text style={styles.statText}>IPv4 Privada</Text></View>
                </View>
                <View style={styles.actionsRow}>
                  ...
                  <View style={styles.cardControlButtons}>
                    <TouchableOpacity style={styles.iconActionButton} activeOpacity={0.7} onPress={() => handleOpenAddSubnet(net.id)}><Feather name="plus-circle" size={16} color="#0A84FF" /></TouchableOpacity>
                    <TouchableOpacity style={styles.iconActionButton} activeOpacity={0.7} onPress={() => handleDeleteNetwork(net.id)}><Feather name="trash-2" size={16} color="#FF453A" /></TouchableOpacity>
                  </View>
                </View>
                {isExpanded && (
                      ...
                      net.subnets.map((sub) => (
                        <View key={sub.id} style={styles.subnetItem}>
                          ...
                          <View style={styles.subnetSide}>
                            <View style={styles.deviceCountBadge}><Feather name="cpu" size={12} color="#30D158" /><Text style={styles.deviceCountText}>{(sub as any).device_count || 0}</Text></View>
                            <TouchableOpacity style={styles.deleteSubnetBtn} activeOpacity={0.7} onPress={() => handleDeleteSubnet(net.id, sub.id)}><Feather name="trash-2" size={14} color="rgba(255, 69, 58, 0.7)" /></TouchableOpacity>
                          </View>
                        </View>
                      ))
                    )}
...
const styles = StyleSheet.create({
  ...
  headerBadge: { fontFamily: 'Poppins_600SemiBold', fontSize: 11, color: '#0A84FF', letterSpacing: 1.5, marginBottom: 2 },
  ...
  networkList: { gap: 18 },
  networkCard: { padding: 20, borderRadius: 24, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.08)', backgroundColor: 'rgba(0, 0, 0, 0.5)' },
  cardTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 },
  networkInfo: { flex: 1 },
  networkTitleContainer: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 6 },
  networkName: { fontFamily: 'Poppins_700Bold', fontSize: 18, color: '#FFFFFF' },
  cidrBadge: { backgroundColor: 'rgba(10, 132, 255, 0.15)', borderWidth: 1, borderColor: 'rgba(10, 132, 255, 0.3)', paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8 },
  cidrText: { fontFamily: 'Poppins_600SemiBold', fontSize: 12, color: '#0A84FF' },
  networkDesc: { fontFamily: 'Poppins_400Regular', fontSize: 13, color: 'rgba(255, 255, 255, 0.5)' },
  statsRow: { flexDirection: 'row', gap: 14, paddingVertical: 12, borderTopWidth: 1, borderBottomWidth: 1, borderColor: 'rgba(255, 255, 255, 0.06)', marginBottom: 14 },
  statItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  statText: { fontFamily: 'Poppins_400Regular', fontSize: 12, color: 'rgba(255, 255, 255, 0.7)' },
  actionsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  expandButton: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 6 },
  expandButtonText: { fontFamily: 'Poppins_600SemiBold', fontSize: 13, color: '#FFFFFF' },
  cardControlButtons: { flexDirection: 'row', gap: 8 },
  iconActionButton: { width: 34, height: 34, borderRadius: 10, backgroundColor: 'rgba(255, 255, 255, 0.05)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.06)' },
  subnetsContainer: { marginTop: 18, paddingTop: 16, borderTopWidth: 1, borderColor: 'rgba(255, 255, 255, 0.08)', gap: 10 },
  subnetHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  subnetSectionTitle: { fontFamily: 'Poppins_600SemiBold', fontSize: 11, color: 'rgba(255, 255, 255, 0.4)', letterSpacing: 1 },
  addSubnetSmallBtn: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  addSubnetSmallText: { fontFamily: 'Poppins_600SemiBold', fontSize: 12, color: '#0A84FF' },
  emptySubnetText: { fontFamily: 'Poppins_400Regular', fontSize: 13, color: 'rgba(255, 255, 255, 0.4)', fontStyle: 'italic', paddingVertical: 8 },
  subnetItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'rgba(255, 255, 255, 0.03)', borderRadius: 14, padding: 12, borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.05)' },
  subnetMain: { flex: 1, paddingRight: 10 },
  subnetTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 2 },
  subnetName: { fontFamily: 'Poppins_600SemiBold', fontSize: 13, color: '#FFFFFF' },
  subnetCidrBadge: { backgroundColor: 'rgba(191, 90, 242, 0.15)', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 },
  subnetCidrText: { fontFamily: 'Poppins_600SemiBold', fontSize: 11, color: '#BF5AF2' },
  subnetDesc: { fontFamily: 'Poppins_400Regular', fontSize: 11, color: 'rgba(255, 255, 255, 0.45)' },
  subnetSide: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  deviceCountBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(48, 209, 88, 0.1)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  deviceCountText: { fontFamily: 'Poppins_600SemiBold', fontSize: 11, color: '#30D158' },
