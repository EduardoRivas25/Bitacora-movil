...
import { MOCK_CONFIGS, MOCK_DEVICES } from '../../mock/data';
...
export default function ConfigScreen() {
  ...
  const [configs, setConfigs] = useState<DeviceConfig[]>(MOCK_CONFIGS);
  const [selectedConfig, setSelectedConfig] = useState<DeviceConfig | null>(MOCK_CONFIGS[0]);
  ...
  const currentDevice = MOCK_DEVICES.find(d => d.id === selectedDeviceId) || MOCK_DEVICES[0];

  const handleSimulateFileUpload = () => {
    // Simula la selección de archivo de configuración
    const dummyFileNames = [
      'running-config-backup.cfg',
      'cisco-sw-core-vlan.bak',
      'mikrotik-routes-ospf.rsc',
      'fortigate-firewall-rules.conf',
    ];
    const picked = dummyFileNames[Math.floor(Math.random() * dummyFileNames.length)];
    setUploadedFileName(picked);
    if (!configName) setConfigName(`Backup automático ${picked.split('.')[0]}`);
    if (!configContent) {
      setConfigContent(
...
!
  ...
  const handleSaveConfig = () => {
    ...
    const newCfg: DeviceConfig = {
      ...
      device_type: currentDevice.name.includes('Switch') ? 'Switch' : 'Router',
      file_name: uploadedFileName || `${configName.toLowerCase().replace(/\s+/g, '-')}.cfg`,
      file_size: `${(Math.random() * 8 + 4).toFixed(1)} KB`,
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerSubtitle}>
            Almacena, visualiza y respalda scripts de configuración de switches, routers y firewalls
          </Text>
        </View>
        ...
        <View style={[styles.mainLayout, isDesktop && styles.mainLayoutDesktop]}>
          ...
          {/* COLUMNA IZQUIERDA: Formulario de Carga y Backups Recientes */}
          <View style={[styles.leftColumn, isDesktop && styles.leftColumnDesktop]}>
            
            {/* Tarjeta: Cargar Configuración */}
            <BlurView intensity={30} tint="dark" style={styles.glassCard}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.deviceChipsRow}>
                {MOCK_DEVICES.map(dev => {
                  ...
                  return (
                    ...
                    >
                      <Feather 
                        name={dev.name.includes('Switch') ? 'server' : 'radio'} 
                        size={12} 
              ...
              <View style={styles.formRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    placeholder="ej. Backup VLANs & Trunk"
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    placeholder="ej. Rutas OSPF hacia Edif B"
                    placeholderTextColor="rgba(255, 255, 255, 0.25)"
                    style={styles.input}
                    value={configDesc}
              ...
              {/* Dropzone de Archivo */}
              <TouchableOpacity 
                style={[styles.dropzone, uploadedFileName && styles.dropzoneActive]}
                activeOpacity={0.7}
              ...
              >
                ...
                <Text style={styles.dropzoneTitle}>
                  {uploadedFileName ? `Archivo cargado: ${uploadedFileName}` : 'Seleccionar archivo de configuración'}
                </Text>
                <Text style={styles.dropzoneSubtitle}>
                  Formatos soportados: .sh, .bak, .txt, .cfg, .rsc (Máx 1 MB)
                </Text>
              </TouchableOpacity>

              {/* Editor de código rápido / Pegar texto */}
              ...
              >
                ...
                <Text style={styles.submitBtnText}>Enviar configuración</Text>
              </TouchableOpacity>
            </BlurView>

            {/* Tarjeta: Backups Recientes */}
            <BlurView intensity={30} tint="dark" style={[styles.glassCard, { marginTop: 20 }]}>
              ...
              <View style={styles.backupList}>
                {configs.map((cfg, idx) => {
                  ...
                  return (
                    <View key={cfg.id} style={[styles.backupItem, isViewing && styles.backupItemActive]}>
                      <View style={{ flex: 1, paddingRight: 10 }}>
                        <Text style={styles.backupName}>{cfg.name}</Text>
                        <Text style={styles.backupMeta}>
                          {cfg.device_name} • {cfg.file_name} ({cfg.file_size})
                        </Text>
                      </View>

                      <View style={styles.backupActions}>
                        <TouchableOpacity 
                          style={[styles.actionPill, styles.actionPillView]}
                          activeOpacity={0.7}
                          onPress={() => setSelectedConfig(cfg)}
                        >
                          <Text style={styles.actionPillViewText}>Ver</Text>
                        </TouchableOpacity>
                        ...
                        >
                          <Text style={styles.actionPillDeleteText}>Eliminar</Text>
                        </TouchableOpacity>
                      </View>
                    </View>
                  );
          ...
          {/* COLUMNA DERECHA: Visor de Código / Terminal Estilo Mac */}
            ...
            <View style={styles.terminalWindow}>
              ...
              <View style={styles.terminalHeader}>
                ...
                <Text style={styles.terminalTitle} numberOfLines={1}>
                  {selectedConfig ? selectedConfig.file_name : 'Sin archivo cargado'}
                </Text>
                ...
                {selectedConfig && (
                  <TouchableOpacity 
                    ...
                    activeOpacity={0.7}
                  >
                    <Feather name={copiedSuccess ? "check" : "copy"} size={13} color="#FFFFFF" />
                    <Text style={styles.copyBtnText}>{copiedSuccess ? "Copiado" : "Copiar"}</Text>
                  </TouchableOpacity>
                )}
              </View>

              {/* Cuerpo del Visor de Código */}
              {selectedConfig ? (
                  ...
                  <View style={styles.codeContentRow}>
                    ...
                    <View style={styles.lineNumbersCol}>
                      {selectedConfig.content.split('\n').map((_, index) => (
                        <Text key={index} style={styles.lineNumberText}>
                          {index + 1}
                        </Text>
                      ))}
                    </View>
                    ...
                    <View style={styles.codeLinesCol}>
                      {selectedConfig.content.split('\n').map((line, index) => {
                        ...
                        else if (isCommand) color = '#60A5FA';
                        else if (isProperty) color = '#34D399';

                        return (
                          <Text key={index} style={[styles.codeLineText, { color }]}>
                            {line || ' '}
                          </Text>
                        );
                      })}
                ...
                <View style={styles.emptyTerminal}>
                  <Feather name="file-text" size={54} color="rgba(255, 255, 255, 0.15)" />
                  <Text style={styles.emptyTerminalTitle}>Sin archivo cargado</Text>
                  <Text style={styles.emptyTerminalSubtitle}>
                    Selecciona un backup de la lista o sube un nuevo archivo de configuración para visualizar sus comandos CLI
                  </Text>
                </View>
              )}
            </View>
