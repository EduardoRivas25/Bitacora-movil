...
export default function DeviceListScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [devs, blds, subs] = await Promise.all([api.fetchDevices(), api.fetchBuildings(), api.fetchAllSubnetsFlat()]);
  ...
  const handleSelectBuilding = (bldId: string) => {
    ...
    if (bld) { setDevLat(bld.latitude.toString()); setDevLng(bld.longitude.toString()); if (bld.departments.length > 0) { setDevDept(bld.departments[0].id); setDevLocation(`${bld.name} - ${bld.departments[0].name}`); } }
  };
  ...
  const handleSaveDevice = async () => {
    if (!devName || !devMac || !devIp || !devSubnet) return;
    const bld = buildings.find(b => b.id === devBuilding);
    const dept = bld?.departments.find(d => d.id === devDept);
    try {
      await api.createDevice({
        name: devName, mac_address: devMac.toUpperCase(), manufacturer: devBrand || 'Genérico',
        location: devLocation || `${bld?.name || 'Edificio'} - ${dept?.name || 'General'}`,
        ipv4_address: devIp, subnet_id: devSubnet, description: devDesc || 'Dispositivo registrado.',
        latitude: parseFloat(devLat) || bld?.latitude || 19.4326, longitude: parseFloat(devLng) || bld?.longitude || -99.1332,
        building_id: devBuilding || undefined, department_id: devDept || undefined,
      });
      setShowModal(false); setDevName(''); setDevMac(''); setDevBrand(''); setDevLocation(''); setDevIp(''); setDevDesc('');
      await loadData();
    } catch (err) { console.error(err); }
  };

  const handleDeleteDevice = async (id: string) => {
    try { await api.deleteDevice(id); await loadData(); } catch (err) { console.error(err); }
  };
    ...
    <LinearGradient colors={['#050505', '#121212']} style={styles.container}>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingHorizontal: isTablet ? '10%' : '5%' }]} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.addButton} activeOpacity={0.8} onPress={() => setShowModal(true)}><Feather name="plus" size={18} color="#000000" /><Text style={styles.addButtonText}>Nuevo Equipo</Text></TouchableOpacity>
        </View>
        ...
        <GlassModal visible={showModal} onClose={() => setShowModal(false)} title="Registrar Dispositivo" subtitle="Asocia un nuevo equipo a una subred activa">
          ...
          <TextInput placeholder="ej. Switch Distribución Edificio B" placeholderTextColor="rgba(255, 255, 255, 0.25)" style={styles.input} value={devName} onChangeText={setDevName} />
          <View style={styles.formRow}><View style={{ flex: 1 }}><Text style={styles.inputLabel}>Dirección IPv4 *</Text><TextInput placeholder="ej. 10.0.10.20" placeholderTextColor="rgba(255, 255, 255, 0.25)" style={styles.input} value={devIp} onChangeText={setDevIp} /></View><View style={{ flex: 1 }}><Text style={styles.inputLabel}>Dirección MAC *</Text><TextInput placeholder="AA:BB:CC:DD:EE:FF" placeholderTextColor="rgba(255, 255, 255, 0.25)" autoCapitalize="characters" style={styles.input} value={devMac} onChangeText={setDevMac} /></View></View>
          <View style={styles.formRow}><View style={{ flex: 1 }}><Text style={styles.inputLabel}>Fabricante / Marca</Text><TextInput placeholder="ej. Cisco / Ubiquiti" placeholderTextColor="rgba(255, 255, 255, 0.25)" style={styles.input} value={devBrand} onChangeText={setDevBrand} /></View><View style={{ flex: 1 }}><Text style={styles.inputLabel}>Ubicación Física</Text><TextInput placeholder="ej. Rack 02 - Piso 1" placeholderTextColor="rgba(255, 255, 255, 0.25)" style={styles.input} value={devLocation} onChangeText={setDevLocation} /></View></View>
          <Text style={styles.inputLabel}>Edificio / Ubicación Física *</Text>
          <View style={styles.buildingSelector}>{buildings.map((bld) => (<TouchableOpacity key={bld.id} style={[styles.buildingOption, devBuilding === bld.id && styles.buildingOptionActive]} onPress={() => handleSelectBuilding(bld.id)}><Text style={[styles.buildingOptionText, devBuilding === bld.id && styles.buildingOptionTextActive]}>{bld.code}</Text></TouchableOpacity>))}</View>
          <View style={styles.formRow}><View style={{ flex: 1 }}><Text style={styles.inputLabel}>Latitud GPS</Text><TextInput placeholder="19.4326" placeholderTextColor="rgba(255, 255, 255, 0.25)" style={styles.input} value={devLat} onChangeText={setDevLat} /></View><View style={{ flex: 1 }}><Text style={styles.inputLabel}>Longitud GPS</Text><TextInput placeholder="-99.1332" placeholderTextColor="rgba(255, 255, 255, 0.25)" style={styles.input} value={devLng} onChangeText={setDevLng} /></View></View>
          <Text style={styles.inputLabel}>Subred de Destino</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>{subnets.map((sub) => (<TouchableOpacity key={sub.id} style={[styles.subnetOption, devSubnet === sub.id && styles.subnetOptionActive]} onPress={() => setDevSubnet(sub.id)}><Text style={styles.subnetOptionText} numberOfLines={1}>{sub.name}</Text></TouchableOpacity>))}</ScrollView>
          ...
          <TouchableOpacity style={styles.modalSubmitButton} activeOpacity={0.8} onPress={handleSaveDevice}><Text style={styles.modalSubmitButtonText}>Guardar en Inventario</Text></TouchableOpacity>
        </GlassModal>
      </ScrollView>
    </LinearGradient>
  );
